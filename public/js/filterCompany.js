var timeout = null;
function getCompany(event, index) {
  ...
  timeout = setTimeout(() => {
    filter = input.value;
    url = url + "/company/filter?name=" + filter;
    // console.log(url);
    // console.log(filter);
    axios
      .get(url)
      .then(function (response) {
        ...
        if (response.data.length > 0) {
          let items = response.data.map((data) => createCompanyItem(data)).join("");
...
function createCompanyItem(company) {
  return `<div class="container">
            <div class="d-flex flex-row justify-content-start align-items-center">
              <div class="ms-2">
              <input class="form-check-input" type="radio" name="companyId" id="${company._id}" value="${company._id}">
              <label class="form-check-label" for="${company._id}">${company.name}</label>
              </div>
              <div class="ms-2">
              <img class="rounded-circle" width="40" height="40" src="${company.logo[0].url}" alt="${company.name}">
              </div>
            </div>
        </div>`;
}
